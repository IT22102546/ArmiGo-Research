...
const SignIn = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      console.log("Sign in with", data, "Remember me:", rememberMe, "Role:", userRole);
      
      const response = await fetch(`${API}/api/auth/mobile-signin`, {
        method: "POST",
        headers: {
      ...
      if (result.success && result.token && result.user) {
        // Store token and user data using Zustand store
        await signIn(result.user, result.token);
        
        Alert.alert("Success", "Signed in successfully!");
        
        // Redirect based on user type or to home screen
        router.replace({
