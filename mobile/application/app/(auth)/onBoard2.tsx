...
import { 
  View, Text, StyleSheet, Image, Animated, TouchableOpacity, 
  ImageSourcePropType, PanResponder, Dimensions 
} from "react-native";
...
const OnBoard2: React.FC = () => {
  ...
  const swipeLeftAnim = useRef(new Animated.Value(0)).current; // For next screen
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        // Update swipe animation based on gesture direction
        const progress = Math.max(0, Math.min(1, Math.abs(gestureState.dx) / 200));
        
        if (gestureState.dx < 0) {
          // Swiping left - next screen animation
          swipeLeftAnim.setValue(progress);
          contentAnim.setValue(progress);
          swipeRightAnim.setValue(0); // Reset right swipe
        } else {
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        if (Math.abs(gestureState.dx) > 100 && gestureState.dx < 0) {
          // Swipe left confirmed - go to next screen
          handleSwipeComplete();
        } else if (Math.abs(gestureState.dx) > 100 && gestureState.dx > 0) {
  ...
  // Rotation animations only for swipe right (back)
  const triangleRotation = swipeRightAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ['0deg', '45deg'],
  });

  const vectorRotation = swipeRightAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ['0deg', '-30deg'],
  });

  const eclipseRotation = swipeRightAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ['0deg', '60deg'],
  });

  // Animation transforms for decorative elements
  // Triangle - same for both directions
  ...
  // Vector - different behavior for swipe directions
  const vectorTransform = {
    transform: [
      ...
      {
        rotate: vectorRotation, // Only rotates on swipe right
      },
      // Movement toward screen only on swipe left (next)
      {
        translateX: swipeLeftAnim.interpolate({
          inputRange: [0, 1],
          outputRange: [0, -50], // Move left toward center during swipe left
        }),
      },
      {
        translateY: swipeLeftAnim.interpolate({
          inputRange: [0, 1],
          outputRange: [0, 30], // Move down toward center during swipe left
        }),
      },
    ],
  ...
  // Eclipse - different behavior for swipe directions
  const eclipseTransform = {
    transform: [
      ...
      {
        rotate: eclipseRotation, // Only rotates on swipe right
      },
      // Movement toward screen only on swipe left (next)
      {
        translateX: swipeLeftAnim.interpolate({
          inputRange: [0, 1],
          outputRange: [0, 40], // Move right toward center during swipe left
        }),
      },
      {
        translateY: swipeLeftAnim.interpolate({
          inputRange: [0, 1],
          outputRange: [0, -40], // Move up toward center during swipe left
        }),
      },
    ],
  ...
  const handleSwipeComplete = () => {
    ...
    console.log('🔄 Swipe complete - navigating to OnBoard3');

    // Complete the swipe animation (left - next screen)
    Animated.parallel([
      Animated.timing(swipeLeftAnim, {
        toValue: 1,
        duration: 200,
        useNativeDriver: true,
      }),
  ...
  const handleSwipeBack = () => {
    ...
    console.log('🔄 Swipe back - navigating to OnBoard1');
    ...
    Animated.parallel([
      ...
      swipeRightAnim.setValue(0);
      contentAnim.setValue(0);
      // Navigate to OnBoard1 after animation completes
      router.push('/onBoard1');
    });
  };

  const handleNextPress = (): void => {
    console.log('Next button pressed - navigating to OnBoard3');
  ...
  return (
    <View style={styles.container}>
              ...
              <MaskedView
                ...
                maskElement={
                  <View style={styles.maskElement}>
                    <Text style={styles.gradientTitleMask}>Take Exams Securely</Text>
                  </View>
                }
              >
                <LinearGradient
      ...
      {/* Skip Button OUTSIDE the PanResponder */}
      <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  // Skip Button Styles - Bottom Left Corner
  skipButtonContainer: {
    position: "absolute",
    bottom: 30,
    left: 30,
