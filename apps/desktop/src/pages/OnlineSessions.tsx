  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const endpoint = childId ? `/api/v1/users/my-online-sessions?childId=${encodeURIComponent(childId)}` : '/api/v1/users/my-online-sessions';
  ...
  if (loading) {
    return (
      <div className="flex-1 overflow-y-auto p-8 page-enter">
        <div className="mb-8"><div className="h-8 w-48 shimmer rounded-lg mb-2" /><div className="h-4 w-64 shimmer rounded-lg" /></div>
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white rounded-2xl p-5 card-shadow border border-slate-100">
              ...
              <div className="space-y-2"><div className="h-4 w-3/4 shimmer rounded" /><div className="h-4 w-1/2 shimmer rounded" /></div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8 page-enter">
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Online Sessions</h1>
          <p className="text-sm text-slate-500 mt-1">
            {sessions.length > 0 ? `${sessions.length} virtual therapy appointments` : 'Your virtual therapy appointments'}
          </p>
        </div>
      </div>
      ...
      {sessions.length === 0 ? (
        <div className="bg-white rounded-2xl p-16 text-center card-shadow-lg border border-slate-100">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-50 to-blue-50 flex items-center justify-center mx-auto mb-5">
            <Video size={36} className="text-indigo-500" />
          </div>
          <p className="text-xl font-semibold text-slate-800">No online sessions</p>
          <p className="text-sm text-slate-400 mt-2">Your virtual sessions will appear here when scheduled</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
          {sessions.map((s: any) => {
            ...
            return (
              <div key={s.id} className={`bg-white rounded-2xl p-5 card-shadow border hover-lift ${isOngoing ? 'border-emerald-200 ring-1 ring-emerald-100' : 'border-slate-100'}`}>
                <div className="flex items-center justify-between mb-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isOngoing ? 'gradient-primary' : 'bg-indigo-50'}`}>
                    <Video size={18} className={isOngoing ? 'text-white' : 'text-indigo-600'} />
                  </div>
                  <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full ${colors.bg}`}>
                    <div className={`w-1.5 h-1.5 rounded-full ${colors.dot} ${isOngoing ? 'animate-pulse' : ''}`} />
                ...
                {isOngoing && s.meetingLink && (
                  <a href={s.meetingLink} target="_blank" rel="noopener noreferrer"
                     className="mt-4 flex items-center justify-center gap-2 w-full py-2.5 gradient-primary text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity">
