...
// Types
interface CreateTeacherTransferDto {
...
export default function CreateTransferRequest() {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof CreateTeacherTransferDto, string>>
  >({});
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Fetch subjects
      try {
        const subjectsData = await fetchBackendData(
  ...
  const checkExistingRequest = async () => {
    try {
      const response = await apiFetch("/api/v1/transfer/my-requests", {
        method: "GET",
      });
      ...
      if (response.ok) {
        ...
        setHasExistingRequest(hasActive);
      }
    } catch (error) {
  ...
  // Validate form - CRITICAL: matches backend validation
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof CreateTeacherTransferDto, string>> =
      {};
    ...
    try {
      if (hasExistingRequest) {
        Alert.alert(
          ...
          "You already have an active transfer request. Please edit or cancel it before creating a new one.",
      ...
      const submitData = {
        ...
        currentDistrict: formData.currentDistrict.trim(), // String district name
        currentZone: formData.currentZone.trim(), // String zone name
        fromZone: formData.fromZone.trim(), // String zone name
        toZones: formData.toZones.map((zone) => zone.trim()), // Array of zone names
        subject: formData.subject.trim(), // String subject name
        medium: formData.medium.trim(), // String medium name
        ...
        additionalRequirements:
          formData.additionalRequirements?.trim() || undefined,
      ...
      // Send request
      const response = await apiFetch("/api/v1/transfer", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.errorCode === "ACTIVE_TRANSFER_EXISTS") {
            setHasExistingRequest(true);
            throw new Error("You already have an active transfer request.");
          }
          ...
          if (errorData.errorCode === "SUBJECT_REQUIRED") {
            throw new Error(
              "Subject is required. Please select a valid subject."
            );
          }

          if (errorData.errorCode === "MEDIUM_REQUIRED_TRANSFER") {
            throw new Error(
              "Medium is required. Please select a valid medium."
            );
          }
      ...
      Alert.alert(
        ...
        "Your transfer request has been submitted successfully.\n\nTracking ID: " +
          (result.data?.uniqueId || result.uniqueId || "N/A"),
        [
          {
      ...
      if (error.message?.includes("active transfer request")) {
        setHasExistingRequest(true);
        Alert.alert("Active Request Found", error.message, [
          { text: "OK", style: "cancel" },
        ]);
        return;
      }
      ...
      Alert.alert(
        "Submission Failed",
        error.message || "Failed to submit transfer request."
      );
    } finally {
      setSaving(false);
    }
  ...
  const renderField = (
    label: string,
    value: any,
    fieldKey: keyof CreateTeacherTransferDto,
  ...
  // Filter zones for current district
  const getZonesForCurrentDistrict = () => {
    if (!formData.currentDistrict) return [];
    // This assumes your zones have district info. If not, use all zones.
    return zones;
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.existingRequestContainer}>
          <View style={styles.existingRequestContent}>
            ...
            <Text style={styles.existingRequestMessage}>
              You already have an active transfer request. Please manage your
              existing request before creating a new one.
            </Text>
            ...
            >
              <Text style={styles.viewRequestButtonText}>View My Requests</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
          ...
          <View style={styles.instructionsContent}>
            ...
            <View style={styles.instructionsText}>
              ...
              <Text style={styles.instructionsSubtitle}>
                All fields marked with * are required. Please ensure zone names
                match exactly.
              </Text>
            </View>
          </View>
...
// Styles (same as before, just include the key ones)
