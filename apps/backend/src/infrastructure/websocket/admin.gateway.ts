  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: AuthenticatedSocket, ...args: any[]) {
    try {
      ...
      // CRITICAL: Verify token with proper options
      const payload = await this.jwtService.verifyAsync(token, {
        secret: this.configService.get<string>("jwt.secret"),
        algorithms: ["HS256"], // Only allow expected algorithm
        ignoreExpiration: false, // Reject expired tokens
        clockTolerance: 0, // No clock tolerance
        issuer: this.configService.get<string>("jwt.issuer"), // Validate issuer
        audience: this.configService.get<string>("jwt.audience"), // Validate audience
      });

      // Validate payload structure
      if (!payload || !payload.sub || !payload.role) {
        this.logger.warn(
          `Client ${client.id} provided token with invalid payload`
        );
      ...
      // Only allow admin, super_admin, and teacher connections to admin namespace
      const allowedRoles = [
        "ADMIN",
        "SUPER_ADMIN",
        "INTERNAL_TEACHER",
      ...
      if (!allowedRoles.includes(payload.role)) {
        this.logger.warn(
          `Client ${client.id} with role ${payload.role} attempted to connect to admin namespace`
        );
      ...
      if (error instanceof Error) {
        if (error.name === "TokenExpiredError") {
          ...
          client.emit("error", { message: "Invalid token signature" });
        } else {
  ...
  broadcastStatsUpdate(stats: any) {
    this.server.to("role_ADMIN").to("role_SUPER_ADMIN").emit("statsUpdated", {
      data: stats,
      timestamp: new Date().toISOString(),
    });
  }
  ...
  notifySessionCreated(sessionData: {
    ...
    this.server.to("role_ADMIN").to("role_SUPER_ADMIN").emit("sessionCreated", {
      data: sessionData,
      timestamp: new Date().toISOString(),
    });
  }

  notifySessionTerminated(sessionData: {
    sessionId: string;
    userId: string;
    terminatedBy: string;
    reason?: string;
  ...
  notifyUserSessionsTerminated(data: {
    userId: string;
    userName: string;
    sessionCount: number;
    terminatedBy: string;
