...
export class PublicationsService {
  ...
  async purchase(publicationId: string, userId: string) {
    ...
    if (existingPurchase) {
      throw new ConflictException("Publication already purchased");
    }
  ...
  async getDownloadUrl(publicationId: string, userId: string) {
    ...
    if (purchase.accessExpiry && purchase.accessExpiry < new Date()) {
      throw new ForbiddenException("Access to this publication has expired");
    }
    ...
    // In production, generate signed S3 URL here
    // For now, return the file URL directly
    const signedUrl = purchase.publication.fileUrl; // TODO: Generate signed URL
   ...
   * Note: Teachers can currently delete any publication.
   * TODO: Add createdById field to track ownership and restrict teachers to their own publications
   */
  async remove(id: string, user?: any) {
    ...
    const deletedPublication = await this.prisma.publication.delete({ where: { id } });
