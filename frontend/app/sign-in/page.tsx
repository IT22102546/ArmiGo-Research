...
export default function SignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Try to login - backend will determine the role
      await login({ 
        email, 
        password
      });
      
      // Login successful - redirect will happen automatically based on role
    } catch (err: any) {
      ...
      >
        ...
        <div className="relative z-10 text-center px-12">
          <h2 className="text-4xl font-bold text-white mb-2">
            Welcome to{" "}
            <span className="block">LearnUp Platform</span>
          </h2>
          ...
          <div className="mt-8 flex justify-center">
            <div className="w-full max-w-2xl rounded-lg shadow-2xl bg-white/10 p-8">
              <p className="text-white text-lg">
                Connect with teachers, manage classes, and enhance learning experiences
              </p>
            </div>
          </div>
        </div>
